...
class DomesticData {
    ...
    init(params: String, tableView: UITableView, req: String) {
        
        if params == "locale" {
            locations.forEach{ self.callAPI(locale: $0, requestParam: params, tableView: tableView, req: req)}
        } else if params == "nation" {
            foreignLocations.forEach{ self.callAPI(locale: $0, requestParam: params, tableView: tableView, req: req)}
        }
    }
    func callAPI(locale: String, requestParam: String, tableView: UITableView, req: String) {
